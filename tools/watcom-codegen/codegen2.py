...
class CFormatter:
    def __init__(self, indentation=4, functions_vararg=False):
        self.indentation = indentation
        self.functions_vararg = functions_vararg
    ...
    def type_to_str(self, dumped_type: DumpedType, depth=0, east=True, variable=False) -> str:
        s = self.type_name_to_str(dumped_type, None, depth=depth, east=east, variable=variable)
        return s

    def type_name_to_str(self, dumped_type: DumpedType, name: Optional[str], depth=0, east=True, variable=False) -> str:
        if type(dumped_type) == DumpedTypedefType:
            prefix, suffix = dumped_type.name, ""
        else:
            prefix, suffix = self.type_to_prefix_suffix(dumped_type, depth=depth, east=east, variable=variable)
    ...
    def type_to_prefix_suffix(self, dumped_type: DumpedType, depth=0, east=True, variable=False) -> tuple[str, str]:
        if type(dumped_type) == DumpedPointerType:
            return self.pointer_type_name_to_prefix_suffix(dumped_type.ptr, depth=depth, east=east, variable=variable)
            ...
            if args and self.functions_vararg:
                args.append("...")
            args_str = ", ".join(args)
            prefix = "".join(self.type_to_str(dumped_type.return_type(), depth=depth, east=east, variable=variable)) + "(*"
            suffix = f")({args_str})"
            return prefix, suffix
    ...
    def function_to_ida_str(self, fn: DumpedFunction, name: Optional[str]=None, east=True, ret_east=False) -> str:
        ret_str = self.type_to_str(fn.return_type(), east=ret_east)
        if not (ret_str[-1:] == "*" and ret_east):
            ret_str += " "
        args_str_elems = []
        ...
        for arg, raw_arg in itertools.zip_longest(fn.arguments(), fn.raw_data["args"]):
            ...
            args_str_elems.append(self.type_name_to_str(arg.kind, arg_name, east=east))
        if fn.is_vararg():
            args_str_elems.append("...")
        name = name if name else fn.name
        callconv = "__usercall" if fn.raw_data["args"] else "__cdecl"
    ...
    def typed_name_to_format_str(self, t: DumpedType, name: str) -> Optional[tuple[str, list[str]]]:
        if type(t) in (DumpedPointerType, DumpedArrayType):
            ...
            if t.name in ("char", ):
                return "'%c'", [name]
            # elif t.name in ("tS32", "tS16", "tS8", "int", "short", "br_int_32", "br_int_16", "br_int_8", "br_boolean", ):
            elif t.name in ("long", "int", "short", "signed char", ):
                return "%d", [name]
            # elif t.name in ("tU32", "tU16", "tU8", "br_uint_8", "br_uint_16", "br_uint_32", "unsigned long", "unsigned int", ):
            elif t.name in ("unsigned long", "unsigned int", "unsigned short", "unsigned char", ):
                return "%u", [name]
            elif t.name in ("float", "double", "br_float", "br_scalar", ):
                return "%f", [name]
            elif t.name in ("br_angle", ):
                return "%d", [name]
            elif t.name in ("size_t", "br_size_t", ):
                return "%u", [name]
            else:
    ...
    def pointer_type_name_to_prefix_suffix(self, dumped_type_ptr: DumpedType, depth=0, east=True, variable=False) -> str:
...
class DethRaceProjectGenerator:
    def __init__(self, modules: list[DumpedModule], basepath: Path, indentation: int, formatter: CFormatter):
        ...
        self.duplicate_symbol_names = self._find_duplicate_symbol_names()
    ...
    def module_to_filename(self, module: DumpedModule, new_suffix: str) -> str:
        return (self.basepath / module.name[3:].replace("\\", "/")).with_suffix(new_suffix)

    def _generate_h_file(self, module: DumpedModule):
        filename = self.module_to_filename(module, ".h")
        logger.debug("Writing header for module '%s' to '%s'", module.name, filename)
        mkdir_p(os.path.dirname(filename))
        name = os.path.basename(filename)
        guard_name = f"_{name.upper().replace('.', '_')}_"

        with filename.open("w") as h_file:
            ...
            for fn in module.iter_functions():
                if not self.is_function_ok_to_add(fn):
                    h_file.write("#if 0\n")
                unique_name = self.modify_symbol_name(fn.name, module)
                h_file.write(self.get_h_function_prefix(fn, unique_name))
                h_file.write(self.formatter.function_to_str(fn, name=unique_name, callconv=self.get_function_call_convention(fn, unique_name)))
                h_file.write(";\n")
                if not self.is_function_ok_to_add(fn):
                    h_file.write("#endif\n")
                h_file.write("\n")
    ...
    def _generate_c_file(self, module: DumpedModule):
        filename = self.module_to_filename(module, ".c")
        logger.debug("Writing .c file for module '%s' to '%s'", module.name, filename)

        with filename.open("w") as c_file:
            c_file.write(f"#include \"{filename.with_suffix('.h').name}\"\n\n")
            ...
            for fn in module.iter_functions():
                ...
                c_file.write(self.formatter.function_to_str(fn, name=unique_name, callconv=self.get_function_call_convention(fn, unique_name)))
                c_file.write(" {\n")
    ...
    def _find_duplicate_symbol_names(self) -> list[str]:
        symbols_found = set()
    ...
    def _get_function_prefix(self, fn: DumpedFunction, name: Optional[str]) -> str:
        text = ""
        text += f"// Offset: {fn.raw_data['offset']:d}\n"
        text += f"// Size: 0x{fn.raw_data['size']:x}\n"
        text += f"//IDA: {self.formatter.function_to_ida_str(fn)}\n"
        if name:
